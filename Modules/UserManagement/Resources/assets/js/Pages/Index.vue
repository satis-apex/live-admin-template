    ...
    ElMessageBox.confirm("It will permanently delete. Continue?", "Warning", {
        ...
        callback: (action) => {
            if (action == "confirm") {
                ...
                formData.delete(route("userManagement.destroy", data.id), {
                    preserveScroll: true,
                    onSuccess: () => {
                        formData.reset();
                    },
...
const dataList = $ref(iPropsValue("userManagementList"));
watch(
    () => iPropsValue("userManagementList"),
    () => {
        dataList = iPropsValue("userManagementList");
