Vue.mixin({
	methods: {
		setWithExpiry: function (key, value, ttl = 360000) {
			const now = new Date();
		...
		flashMessage: function (Message, type = "info", reload = false) {
		...
		//error,info,success,warning,default
		flashErrorMessage: function (error, type = "info", reload = false) {
			type == "" ? (msgType = "info") : (msgType = type);
		...
		getObjectIndex: function (key, value) {
			for (var i = 0; i < this.length;)
				if (this[i++][key] === value) return --i;
			return -1;
		},
		themeOption: function (key) {
			return App.themeOption[key];
		},
		siteUrl: function (link = "", opt = "") {
			if (opt == "api") return App.siteUrl + "/api/v1/" + link;
			return App.siteUrl + "/" + link;
		},
		getObjectRow: function (jsObj, searchKey, searchValue) {
			let multipleKey;
			let multipleValue;
		...
		wordToCamel: function (str) {
			return str.replace(/\W+(.)/g, function (match, chr) {
				return chr.toUpperCase();
			});
		},
		camelToWord: function (key) {
			return _.startCase(key);
		},
		readableWord: function (key) {
			return _.startCase(key);
		},
		...
		exportTable: function (tblId = "exportTable", fileName = "download") {
		...
		jsonToTableExport: function (jsonValues, headerFields, fileName) {
			let col = [];
		...
		filterObject: function (objectArray, keys) {
			let filteredObj = [];
		...
		smoothScrollIntoView: function (identifier) {
			setTimeout(() => {
		...
		studentAvatar: function (imageLink) {
		...
		ifUrlExist: function (url, callback) {
			if (url == '' || url == null)
				return false
